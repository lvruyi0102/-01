...
const state = { db: null, layer: 'geo', scenario: null, result: null, year: 2026, graph: null, drag: null, rotation: 0 };
...
function initGraph() {
  ...
  state.graph = {
    nodes: names.map((name, i) => ({ id: name, x: 80 + (i % 4) * 150, y: 85 + Math.floor(i / 4) * 130 })),
    links: [['气候风险', '资源供应', .72], ['资源供应', '经济波动', .66], ['经济波动', '供应链韧性', .81], ['人口暴露', '供应链韧性', .54], ['政策干预', '供应链韧性', .48], ['供应链韧性', '行动建议', .88]]
  };
}

function renderGraph() {
  const svg = $('causalGraph'); const { nodes, links } = state.graph;
  svg.innerHTML = links.map(([a, b, w]) => { const s = nodes.find((n) => n.id === a); const t = nodes.find((n) => n.id === b); return `<line x1="${s.x}" y1="${s.y}" x2="${t.x}" y2="${t.y}" stroke-width="${1 + w * 5}"/><text x="${(s.x + t.x) / 2}" y="${(s.y + t.y) / 2 - 8}">${w}</text>`; }).join('') + nodes.map((n) => `<g data-node="${n.id}" transform="translate(${n.x} ${n.y})"><circle r="28"/><text>${n.id}</text></g>`).join('');
  svg.querySelectorAll('[data-node]').forEach((node) => node.onpointerdown = (event) => { state.drag = { id: node.dataset.node, ox: event.offsetX, oy: event.offsetY }; node.setPointerCapture(event.pointerId); });
...
function renderScenario() {
  ...
  $('calibration').innerHTML = `<p>接入源：JSON 世界状态库 / 场景库 / 时间轴参数</p><p>偏差校准：${risk > 70 ? '需提高现实反馈权重' : '当前偏差可控'}</p><p>错误归因：重点检查资源供应与经济波动是否被时间滞后放大。</p>`;
...
async function archiveScenario() {
  ...
  state.scenario = await api(`/api/scenarios/${state.scenario.id}/archive`, { method: 'POST' }); renderAll();
}
...
$('createScenario').onclick = createScenario; $('simulateScenario').onclick = simulateScenario; $('archiveScenario').onclick = archiveScenario;
load().then(drawEarth);
